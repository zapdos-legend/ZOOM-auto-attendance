...
.status-toggle-btn.is-active .status-toggle-knob {
    transform: translateX(75px);
    background: linear-gradient(135deg,#22c55e,#10b981);
    box-shadow:0 10px 24px rgba(34,197,94,.35);
}
.status-toggle-btn:not(.is-active) .label-inactive,
.status-toggle-btn.is-active .label-active {
    color:#fff;
    text-shadow:0 1px 8px rgba(0,0,0,.28);
}
.status-toggle-btn:not(.is-active) .label-active,
.status-toggle-btn.is-active .label-inactive {
...
PERF_CACHE = {}
try:
    PERFORMANCE_CACHE_TTL_SECONDS = int(os.getenv("PERFORMANCE_CACHE_TTL_SECONDS", "2") or "2")
except Exception:
    PERFORMANCE_CACHE_TTL_SECONDS = 2
