...
def build_meeting_report_data(meeting_uuid):
    ...
    if stored_meeting_seconds > 0:
        ...
        if end_minus_start_seconds > 0:
            ...
            meeting_duration_source = "participant_fallback"
    report_duration_seconds = meeting_total_seconds
    report_duration_source = meeting_duration_source
    host_duration_truth_seconds = None
    for row in attendance_rows:
        if row.get("is_host") and int(row.get("rejoin_count") or 0) == 0:
            host_seconds = get_row_effective_total_seconds(row, end_time)
            host_duration_truth_seconds = max(int(host_duration_truth_seconds or 0), int(host_seconds or 0))
    if host_duration_truth_seconds is not None:
        report_duration_seconds = host_duration_truth_seconds
        report_duration_source = "host_duration_truth"
    print(f"REPORT_DURATION_SELECTED meeting_uuid={meeting_uuid} source={report_duration_source} seconds={report_duration_seconds}")
    ...
    summary = {
        ...
        "meeting_duration_minutes": mins_from_seconds(report_duration_seconds),
