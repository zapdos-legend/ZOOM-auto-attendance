...
def finalize_meeting(meeting_uuid, ended_at=None, run_post_actions=True):
    ...
    if run_post_actions:
        try:
            ...
            print(f"⚠️ Smart alert evaluation skipped safely: {e}")
        try:
            with app.app_context():
                auto_send_smart_meeting_report(meeting_uuid, force=False)
        except BaseException as e:
            print(f"⚠️ Smart report auto-send skipped safely: {e}")
    return updated
...
def read_live_snapshot():
    """Return the newest active meeting and enriched live rows.

    Final real fix:
    - Live page must show the current meeting as soon as Zoom sends meeting.started.
    - Participant rows must use the same meeting_uuid used by the webhook.
    - Durations must be calculated at request time, not only when leave event arrives.
    - Rows are sorted by highest live duration first.
    """
    ...
    with db() as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                SELECT *
                FROM meetings
                WHERE COALESCE(meeting_uuid, '') <> ''
                  AND (
                        lower(COALESCE(status, '')) = 'live'
                        OR (end_time IS NULL AND start_time IS NOT NULL AND start_time >= NOW() - INTERVAL '12 hours')
                      )
                ORDER BY
                    CASE WHEN lower(COALESCE(status, '')) = 'live' THEN 0 ELSE 1 END,
    ...
    for row in participants:
        ...
        total_seconds = cast_setting_value(row.get("total_seconds") or 0, int)
        if current_join:
            total_seconds += max(int((server_now - current_join).total_seconds()), 0)
        row["live_total_seconds"] = max(total_seconds, 0)
        row["live_status"] = "LIVE" if current_join else "LEFT"
        row["category"] = "HOST" if row.get("is_host") else ("MEMBER" if row.get("is_member") else "UNKNOWN")
        enriched_participants.append(row)

    enriched_participants.sort(
        key=lambda r: (int(r.get("live_total_seconds") or 0), 1 if r.get("current_join") else 0),
        reverse=True,
    )
...
def build_live_snapshot_payload(include_feed=True):
    ...
    for p in participants:
        ...
        live_total = int(p.get("live_total_seconds") if p.get("live_total_seconds") is not None else (live_total or 0))
        if is_active_now:
            active_now += 1
            if is_known:
                known_active += 1
        ...
        participant_payload.append({
            ...
            "category": p.get("category") or ("HOST" if is_host else ("MEMBER" if is_known else "UNKNOWN")),
            "is_known": is_known,
            "is_host": is_host,
...
try:
    NOTIFICATION_ALERT_TYPE_LABELS.update({
...
try:
    ...
    ALERT_AUTOMATION_COOLDOWN_SECONDS = 1800
try:
    ALERT_AUTOMATION_RUN_EVERY_SECONDS = int(os.getenv("ALERT_AUTOMATION_RUN_EVERY_SECONDS", "90") or "90")
except Exception:
    ALERT_AUTOMATION_RUN_EVERY_SECONDS = 90


def ensure_alert_automation_tables():
    global ALERT_AUTOMATION_TABLES_READY
    if ALERT_AUTOMATION_TABLES_READY:
        return
    try:
        with db() as conn:
            with conn.cursor() as cur:
                ...
                """)
                cur.execute("""
                    CREATE TABLE IF NOT EXISTS alert_automation_cooldowns (
                        alert_key TEXT PRIMARY KEY,
                        alert_type TEXT NOT NULL,
                ...
                """)
                cur.execute("CREATE INDEX IF NOT EXISTS idx_meeting_schedule_time ON meeting_schedule(meeting_time)")
                cur.execute("CREATE INDEX IF NOT EXISTS idx_meeting_schedule_status ON meeting_schedule(status)")
                cur.execute("CREATE INDEX IF NOT EXISTS idx_alert_automation_type ON alert_automation_cooldowns(alert_type)")
            conn.commit()
        ALERT_AUTOMATION_TABLES_READY = True
    except Exception as exc:
        print(f"⚠️ ensure_alert_automation_tables skipped: {exc}")


def _alert_priority_for_type(alert_type):
    return {
        "critical_risk": "critical", "member_risk": "critical", "host_absent": "critical",
        "meeting_health_drop": "warning", "warning_risk": "warning", "declining_trend": "warning",
        "unknown_participant_spike": "warning", "meeting_reminder": "info",
    }.get(str(alert_type or ""), "info")


def _alert_phase_for_type(alert_type):
    if alert_type == "meeting_reminder": return "before"
    if alert_type in ("host_absent", "unknown_participant_spike"): return "during"
    return "after"


def _cooldown_allows_alert(alert_key, alert_type, state, priority):
    ensure_alert_automation_tables()
    now_dt = now_local()
    try:
        with db() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT last_state, last_sent_at FROM alert_automation_cooldowns WHERE alert_key=%s", (alert_key,))
                row = cur.fetchone()
                if row:
                    last_state = str(row.get("last_state") or "")
                    last_sent_at = parse_dt(row.get("last_sent_at"))
                    if last_state == str(state) and last_sent_at and (now_dt - last_sent_at).total_seconds() < ALERT_AUTOMATION_COOLDOWN_SECONDS:
                        return False
                cur.execute("""
                    INSERT INTO alert_automation_cooldowns(alert_key, alert_type, priority, last_state, last_sent_at, updated_at)
                    VALUES (%s,%s,%s,%s,NOW(),NOW())
                    ON CONFLICT(alert_key)
                    DO UPDATE SET alert_type=EXCLUDED.alert_type, priority=EXCLUDED.priority,
                                  last_state=EXCLUDED.last_state, last_sent_at=NOW(), updated_at=NOW()
                """, (alert_key, alert_type, priority, str(state)))
            conn.commit()
    except Exception as exc:
        print(f"⚠️ cooldown check skipped for {alert_key}: {exc}")
        return False
    return True


def trigger_intelligent_alert(alert_type, state="active", title=None, message="", priority=None, member=None, meeting=None, phase=None):
    priority = priority or _alert_priority_for_type(alert_type)
    phase = phase or _alert_phase_for_type(alert_type)
    try:
        if not notification_alert_allowed(alert_type, phase):
            return {"sent": False, "reason": "disabled_by_notification_control", "alert_type": alert_type, "phase": phase}
    except Exception:
        pass
    entity_type, entity_id = _alert_entity(member=member, meeting=meeting)
    alert_key = f"auto:{entity_type}:{entity_id}:{alert_type}"
    if not _cooldown_allows_alert(alert_key, alert_type, state, priority):
        return {"sent": False, "reason": "cooldown", "alert_key": alert_key}
    title = title or f"{priority.upper()} Alert: {str(alert_type).replace('_', ' ').title()}"
    full_message = f"Priority: {priority.upper()}\n{message}" if message else f"Priority: {priority.upper()}"
    try:
        return trigger_alert(member=member, alert_type=alert_type, state=state, message=full_message, title=title, meeting=meeting)
    except Exception as exc:
        print(f"⚠️ trigger_intelligent_alert failed: {exc}")
        return {"sent": False, "reason": str(exc), "alert_key": alert_key}


def _alert_recent_meeting_ids(limit=10):
    try:
        with db() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT meeting_uuid FROM meetings
                    WHERE meeting_uuid IS NOT NULL AND meeting_uuid <> ''
                    ORDER BY COALESCE(end_time, start_time, created_at) DESC
                    LIMIT %s
                """, (limit,))
                return [r.get("meeting_uuid") for r in cur.fetchall() if r.get("meeting_uuid")]
    except Exception as exc:
        print(f"⚠️ recent meetings lookup skipped: {exc}")
        return []


def evaluate_member_risk_alerts():
    meeting_ids = _alert_recent_meeting_ids(12)
    if not meeting_ids:
        return []
    results = []
    try:
        with db() as conn:
            with conn.cursor() as cur:
                name_expr = member_name_sql(conn)
                cur.execute(f"""
                    SELECT m.id, {name_expr} AS display_name, m.email,
                           COUNT(a.id) AS total,
                           SUM(CASE WHEN UPPER(COALESCE(a.final_status,''))='PRESENT' THEN 1 ELSE 0 END) AS present,
                           SUM(CASE WHEN UPPER(COALESCE(a.final_status,''))='LATE' THEN 1 ELSE 0 END) AS late
                    FROM members m
                    LEFT JOIN attendance a ON a.member_id=m.id AND a.meeting_uuid = ANY(%s)
                    WHERE {ACTIVE_MEMBER_SQL}
                    GROUP BY m.id, display_name, m.email
                    ORDER BY display_name
                """, (meeting_ids,))
                rows = cur.fetchall()
                cur.execute("""
                    WITH recent_meetings AS (
                        SELECT meeting_uuid, ROW_NUMBER() OVER (ORDER BY COALESCE(end_time,start_time,created_at) DESC) rn
                        FROM meetings WHERE meeting_uuid = ANY(%s)
                    )
                    SELECT a.member_id,
                           AVG(CASE WHEN rm.rn <= 3 THEN CASE WHEN UPPER(COALESCE(a.final_status,''))='PRESENT' THEN 1 WHEN UPPER(COALESCE(a.final_status,''))='LATE' THEN 0.5 ELSE 0 END END) AS latest_score,
                           AVG(CASE WHEN rm.rn > 3 AND rm.rn <= 6 THEN CASE WHEN UPPER(COALESCE(a.final_status,''))='PRESENT' THEN 1 WHEN UPPER(COALESCE(a.final_status,''))='LATE' THEN 0.5 ELSE 0 END END) AS previous_score
                    FROM attendance a JOIN recent_meetings rm ON rm.meeting_uuid=a.meeting_uuid
                    WHERE a.member_id IS NOT NULL GROUP BY a.member_id
                """, (meeting_ids,))
                trend_rows = {r.get("member_id"): r for r in cur.fetchall()}
        for row in rows:
            total = int(row.get("total") or 0)
            if total <= 0: continue
            score = ((int(row.get("present") or 0) + 0.5 * int(row.get("late") or 0)) / total) * 100
            member = {"id": row.get("id"), "name": row.get("display_name"), "full_name": row.get("display_name"), "email": row.get("email")}
            if score < 50:
                results.append(trigger_intelligent_alert("critical_risk", "critical", "🚨 Critical attendance risk", f"{row.get('display_name')} is at {round(score,2)}% attendance score in recent meetings. Recommendation: immediate follow-up and reminder.", "critical", member=member, phase="after"))
            elif score < 75:
                results.append(trigger_intelligent_alert("warning_risk", "warning", "⚠️ Warning attendance risk", f"{row.get('display_name')} is at {round(score,2)}% attendance score. Recommendation: send early reminder and monitor next meeting.", "warning", member=member, phase="after"))
            trend = trend_rows.get(row.get("id"))
            if trend and trend.get("latest_score") is not None and trend.get("previous_score") is not None:
                latest = float(trend.get("latest_score") or 0); previous = float(trend.get("previous_score") or 0)
                if latest + 0.20 < previous:
                    results.append(trigger_intelligent_alert("declining_trend", "declining", "📉 Declining attendance trend", f"{row.get('display_name')} dropped from {round(previous*100,1)}% to {round(latest*100,1)}% in recent meetings. Recommendation: intervene before risk becomes critical.", "warning", member=member, phase="after"))
    except Exception as exc:
        print(f"⚠️ evaluate_member_risk_alerts skipped: {exc}")
    return results


def _meeting_health_score_from_row(row):
    if not row: return 0.0
    total = int(row.get("unique_participants") or 0)
    if total <= 0:
        total = int(row.get("present_count") or 0) + int(row.get("late_count") or 0) + int(row.get("absent_count") or 0) + int(row.get("unknown_participants") or 0)
    if total <= 0: return 0.0
    present_score = ((int(row.get("present_count") or 0) + 0.5 * int(row.get("late_count") or 0)) / total) * 70
    unknown_penalty = (int(row.get("unknown_participants") or 0) / total) * 20
    host_bonus = 10 if row.get("host_present") else 0
    return max(0.0, min(100.0, round(present_score + host_bonus - unknown_penalty, 2)))


def evaluate_meeting_alerts():
    results = []
    try:
        with db() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT * FROM meetings WHERE meeting_uuid IS NOT NULL AND meeting_uuid <> ''
                    ORDER BY COALESCE(end_time,start_time,created_at) DESC LIMIT 2
                """)
                meetings = cur.fetchall()
        if not meetings: return results
        latest = meetings[0]
        latest_total = int(latest.get("unique_participants") or 0)
        latest_unknown = int(latest.get("unknown_participants") or 0)
        unknown_pct = (latest_unknown / latest_total * 100) if latest_total else 0
        meeting_ref = {"meeting_uuid": latest.get("meeting_uuid"), "id": latest.get("id"), "topic": latest.get("topic")}
        ...
        print(f"⚠️ evaluate_meeting_alerts skipped: {exc}")
    return results


def run_meeting_schedule_reminders():
    ensure_alert_automation_tables()
    results = []
    try:
        now_dt = now_local()
        with db() as conn:
            with conn.cursor() as cur:
                cur.execute("""
                    ...
                    WHERE status='scheduled' AND meeting_time >= NOW() - INTERVAL '10 minutes' AND meeting_time <= NOW() + INTERVAL '1 day'
                    ORDER BY meeting_time ASC LIMIT 30
                """)
                rows = cur.fetchall()
                for row in rows:
                    meeting_time = parse_dt(row.get("meeting_time"))
                    if not meeting_time: continue
                    reminder_minutes = int(row.get("reminder_minutes") or 30)
                    reminder_start = meeting_time - timedelta(minutes=reminder_minutes)
                    if row.get("last_reminder_at") is not None or now_dt < reminder_start or now_dt > meeting_time + timedelta(minutes=10):
                        continue
                    meeting_ref = {"meeting_uuid": f"schedule-{row.get('id')}", "id": row.get("id"), "topic": row.get("title")}
                    results.append(trigger_intelligent_alert("meeting_reminder", "due", "⏰ Meeting reminder", f"'{row.get('title')}' is scheduled at {fmt_dt(meeting_time)}. Recommendation: join on time and keep attendance ready.", "info", meeting=meeting_ref, phase="before"))
                    cur.execute("UPDATE meeting_schedule SET last_reminder_at=NOW(), updated_at=NOW() WHERE id=%s", (row.get("id"),))
            conn.commit()
    except Exception as exc:
        print(f"⚠️ run_meeting_schedule_reminders skipped: {exc}")
    return results


def run_smart_scheduler(force=False):
    global ALERT_AUTOMATION_LAST_RUN_TS
    now_ts = time.time()
    if not force and now_ts - ALERT_AUTOMATION_LAST_RUN_TS < ALERT_AUTOMATION_RUN_EVERY_SECONDS:
        return {"skipped": True, "reason": "throttled"}
    ALERT_AUTOMATION_LAST_RUN_TS = now_ts
    ensure_alert_automation_tables()
    results = []
    try:
        results.extend(run_meeting_schedule_reminders() or [])
        results.extend(evaluate_meeting_alerts() or [])
        results.extend(evaluate_member_risk_alerts() or [])
    except Exception as exc:
        print(f"⚠️ run_smart_scheduler skipped: {exc}")
...
def _smart_alert_scheduler_worker():
    ...
    try:
        # Background thread has no Flask request/app context by default.
        # Without this, url_for() inside push/email alerts can throw:
        # "Working outside of application context".
        with app.app_context():
            run_smart_scheduler(force=False)
    except BaseException as exc:
        print(f"⚠️ smart alert background scheduler skipped safely: {exc}")
    finally:
        ALERT_AUTOMATION_BG_RUNNING = False
...
def smart_alert_automation_after_request(response):
    ...
    try:
        ...
        if path in ("/", "/login", "/favicon.ico") or path.startswith("/static"):
            return response
        if path == "/live" or path.startswith("/api/live") or path.startswith("/api/ai-assistant"):
            return response
