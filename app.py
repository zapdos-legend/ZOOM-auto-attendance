...
def build_member_intelligence_engine(limit=None):
            ...
            cur.execute(
                ...
                WHERE start_time IS NOT NULL
                  ...
                  AND (
                      ...
                      OR LOWER(COALESCE(status, '')) IN ('ended', 'completed', 'closed', 'finalized')
                  )
                ORDER BY start_time ASC NULLS LAST, id ASC
                """
            )
    ...
    for member in members:
        ...
        for meeting in meetings:
            ...
            if source_status in ("PRESENT", "LATE", "ABSENT", "HOST"):
                final_status = source_status
                if final_status == "HOST":
                    duration_pct = 100.0
                elif meeting_seconds > 0:
                    duration_pct = clamp_score((float(total_seconds or 0) / float(meeting_seconds)) * 100.0)
                else:
                    duration_pct = calculate_attendance_status_score(final_status, 0, status_weight=1.0, duration_weight=0.0)
            elif meeting_seconds > 0:
                ...
                duration_pct = clamp_score((float(classified_seconds or 0) / float(meeting_seconds)) * 100.0)
        ...
        # Attendance is the controlling truth: duration/consistency must not make
        # members with poor Attendance Register percentages look Good/Excellent.
