...
(function(){
  ...
  window.fetch = function(input, init){
    ...
    if(url && url.includes("/api/live-summary") && location.pathname !== "/live"){
        return Promise.resolve(new Response(JSON.stringify({ok:true,summary:{}}), {status:200}));
    }
  ...
  window.ZoomAttendanceMotionEngine = {
    ...
    initDurations:function(){
      ...
      function safeInit(el){
        ...
        el.dataset.zaLastUpdate = Date.now();
      }
