    ...
    with db() as conn:
        with conn.cursor() as cur:
            ...
            for row in raw_participants:
                key = row.get("participant_key") or participant_key(row.get("participant_name"), row.get("participant_email"))
...
def _phase41_monthly_attendance_details(period_meta):
    """Read-only duration/last-seen details for monthly intelligence rows.

    Attendance Register remains the source for monthly percentage/status/risk.
    This helper only fills display metadata that the register payload does not carry.
    """
    try:
        year = int(period_meta.get("year") or 0)
        month = int(period_meta.get("month") or 0)
    except Exception:
        return {}
    ...
    with db() as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                ORDER BY COALESCE(a.last_leave, a.current_join, a.first_join, m.start_time) DESC NULLS LAST, a.id DESC
                """,
                (start_day, end_day),
            )
            for row in cur.fetchall() or []:
    ...
    details = defaultdict(lambda: {"seconds": 0, "attended": 0, "last_seen": None})
    for row in best_attendance.values():
        status = str(row.get("final_status") or "").upper().strip()
        if status not in ("PRESENT", "HOST", "LATE"):
            continue
        mid = int(row.get("member_id"))
        details[mid]["seconds"] += int(row.get("total_seconds") or 0)
        details[mid]["attended"] += 1
        seen_at = parse_dt(row.get("last_leave") or row.get("current_join") or row.get("first_join") or row.get("start_time"))
        if seen_at and (details[mid]["last_seen"] is None or seen_at > details[mid]["last_seen"]):
            details[mid]["last_seen"] = seen_at

    return {
        mid: {
            "average_duration": round((values["seconds"] / 60.0) / max(values["attended"], 1), 2) if values["attended"] else 0.0,
            "last_seen": values["last_seen"],
        }
        for mid, values in details.items()
    }
...
def _phase41_build_monthly_member_intelligence(period_meta, limit=None, period_options=None):
    ...
    attendance_details = _phase41_monthly_attendance_details(period_meta)
    ...
    for row in register_data.get("rows") or []:
        ...
        member_details = attendance_details.get(int(row.get("id"))) or {}
        last_seen = member_details.get("last_seen")
        intelligence_rows.append({
            "id": int(row.get("id")),
            "name": row.get("name") or f"Member {row.get('id')}",
            "email": row.get("email") or "",
            "attendance_pct": round(attendance_pct, 2),
            "average_duration": round(float(member_details.get("average_duration") or 0), 2),
            "duration_pct": round(duration_pct, 2),
    ...
    function startGlobalLiveSidebarUpdater(){
        ...
        const tick = async function(reason){
            ...
            try{
                ...
                if(reason === 'boot'){ console.log('LIVE_STATE_HYDRATED'); }
            }catch(_e){}
            finally{ busy=false; }
        };
...
# ===== FINAL WOW PREMIUM MEMBER TREND HELPERS =====
def _za_wow_member_trend_payload(member_id):
    """Read-only, production-safe member trend data for premium profile hero."""
    try:
        with db() as conn:
            with conn.cursor() as cur:
                name_sql = member_name_sql(conn)
                ...
                if trend == "Improving":
                    insight = "Attendance consistency is improving in recent meetings. Keep the current engagement pattern."
                elif trend == "Declining":
                    insight = "Attendance performance is dropping. This member needs follow-up or reminder action."
                else:
                    insight = "Attendance behavior is stable. Continue monitoring for any sudden change."
...
def _za_wow_premium_trend_html(member_id):
    data = _za_wow_member_trend_payload(member_id)
    if not data.get("ok"):
        data = {
            "trend": "Stable",
            "trend_score": 0,
            ...
            "insight": "Not enough reliable data available yet.",
        }

    trend = str(data.get("trend") or "Stable")
    lower = trend.lower()
    icon = "📈" if "improv" in lower else "📉" if "declin" in lower else "➖"
    cls = "improving" if "improv" in lower else "declining" if "declin" in lower else "stable"
    ...
    for idx, p in enumerate(points):
        try:
            s = max(8, min(100, float(p.get("score") or 0)))
        except Exception:
            s = 8
        bcls = "good" if s >= 70 else "warn" if s >= 45 else "bad"
        tip = html_escape(f"{p.get('label','Meeting')} | Score {round(s)}% | {p.get('status','-')} | {p.get('minutes',0)} min")
        bars.append(
            f"<span class='za-wow-bar {bcls}' style='--h:{s}%;animation-delay:{idx*90}ms' data-tip='{tip}'><i>{round(s)}</i></span>"
        )

    return f"""
    <section id="zaWowTrendHero" class="za-wow-trend-hero" title="{html_escape(basis)}">
      <div class="za-wow-orb za-wow-orb-a"></div>
      <div class="za-wow-orb za-wow-orb-b"></div>
      <div class="za-wow-grid">
        <div class="za-wow-left">
          <div class="za-wow-kicker">AI Member Intelligence</div>
          <h2 class="za-wow-title">{icon} {html_escape(trend)} Trend</h2>
          <div class="za-wow-score-line">
            <span class="za-wow-main-score {cls}">{sign}{score:.0f}%</span>
            <span class="za-wow-badge {cls}">{icon} {html_escape(trend)}</span>
          </div>
          <p class="za-wow-insight">{html_escape(insight)}</p>
          <div class="za-wow-meta">
            <span>Older avg: <b>{older:.0f}%</b></span>
            <span>Recent avg: <b>{recent:.0f}%</b></span>
            <span>{html_escape(basis)}</span>
          </div>
        </div>
        <div class="za-wow-chart">
          <div class="za-wow-chart-head">
            <div>
              <strong>Recent Meeting Performance</strong>
              <small>Left = older meetings, right = recent meetings</small>
            </div>
            <em>Hover bars for score/status/minutes</em>
          </div>
          <div class="za-wow-bars">{''.join(bars)}</div>
          <div class="za-wow-mini-grid">
            <div><small>Direction</small><strong>{html_escape(trend)}</strong></div>
            <div><small>Trend Score</small><strong>{sign}{score:.0f}%</strong></div>
            <div><small>Recent Avg</small><strong>{recent:.0f}%</strong></div>
          </div>
        </div>
      </div>
    </section>
...
def _za_elite_member_trend_html(member_id):
    ...
    trend = str(data.get("trend") or "Stable")
    lower = trend.lower()
    if "improving" in lower:
        cls = "up"
        icon = "📈"
    elif "declining" in lower or "drop" in lower:
        cls = "down"
        icon = "📉"
    else:
        cls = "stable"
    ...
    older = float(data.get("older_avg") or 0)
    recent = float(data.get("recent_avg") or 0)
    weighted = float(data.get("weighted_avg") or 0)
    ...
    strength = html_escape(str(data.get("strength") or "Stable Pattern"))

    points = data.get("points") or []
    norm_bars = data.get("normalized_bars") or []
    if not points:
        points = [
            {"score": 20, "label": "No data", "status": "-", "minutes": 0},
            {"score": 35, "label": "No data", "status": "-", "minutes": 0},
        ...
        norm_bars = [20, 35, 55, 70]

    bars = []
    for idx, point in enumerate(points):
        try:
            real_score = max(0, min(100, float(point.get("score") or 0)))
        except Exception:
            real_score = 0
        try:
            visual_height = max(8, min(100, float(norm_bars[idx] if idx < len(norm_bars) else real_score)))
        except Exception:
            visual_height = max(8, real_score)
        ...
        tip = html_escape(
            f"{point.get('label','Meeting')} | Real score {round(real_score)}% | Visual scale {round(visual_height)}% | {point.get('status','-')} | {point.get('minutes',0)} min"
        )
        bars.append(
            f"<span class='za-elite-bar {bcls}' style='--h:{visual_height}%;animation-delay:{idx*95}ms' data-tip='{tip}'><i>{round(real_score)}</i></span>"
        )

    return f"""
    <section id="zaEliteTrendHero" class="za-elite-trend {cls}" title="{basis}">
      <div class="za-elite-bg-orb one"></div>
      <div class="za-elite-bg-orb two"></div>
      <div class="za-elite-grid">
        <div class="za-elite-left">
          <div class="za-elite-kicker">Elite AI Trend Intelligence • Combined Score</div>
          <h2>{icon} {html_escape(trend)}</h2>
          <div class="za-elite-score-row">
            <span class="za-elite-score" data-za-target="{score:.1f}">{sign}{score:.1f}%</span>
            <span class="za-elite-pill">{strength}</span>
          </div>
          <p class="za-elite-insight">{insight}</p>
          <div class="za-elite-action">{action}</div>
          <div class="za-elite-meta">
            <span>Older Avg <b>{older:.0f}%</b></span>
            <span>Recent Avg <b>{recent:.0f}%</b></span>
            <span>Weighted <b>{weighted:.0f}%</b></span>
            <span>{basis}</span>
          </div>
        </div>
        <div class="za-elite-chart">
          <div class="za-elite-chart-head">
            <div>
              <strong>Normalized Recent Performance</strong>
              <small>Real score = attendance/status 60% + duration 40%. Bar height is normalized for clear contrast.</small>
            </div>
            <em>Hover bars for exact score/status/minutes</em>
          </div>
          <div class="za-elite-bars">{''.join(bars)}</div>
          <div class="za-elite-mini">
            <div><small>Direction</small><strong>{html_escape(trend)}</strong></div>
            <div><small>Trend</small><strong>{sign}{score:.1f}%</strong></div>
            <div><small>Next Action</small><strong>{'Reward' if cls == 'up' else 'Follow-up' if cls == 'down' else 'Monitor'}</strong></div>
          </div>
        </div>
      </div>
    </section>
...
.za-premium-bar.good{background:linear-gradient(180deg,#22c55e,#0ea5e9)!important}.za-premium-bar.warn{background:linear-gradient(180deg,#f59e0b,#6366f1)!important}.za-premium-bar.bad{background:linear-gradient(180deg,#ef4444,#7c3aed)!important}
...
#zaWowTrendHero::before{
  ...
  animation:zaWowShine 5.2s ease-in-out infinite;
}
@keyframes zaWowShine{0%,58%{transform:translateX(-115%)}100%{transform:translateX(115%)}}
.za-wow-orb{position:absolute;border-radius:999px;filter:blur(18px);opacity:.55;pointer-events:none;z-index:-1}
.za-wow-orb-a{width:150px;height:150px;right:-40px;top:-40px;background:rgba(34,197,94,.32);animation:zaWowFloatA 7s ease-in-out infinite alternate}
.za-wow-orb-b{width:120px;height:120px;left:-40px;bottom:-40px;background:rgba(99,102,241,.34);animation:zaWowFloatB 8s ease-in-out infinite alternate}
@keyframes zaWowFloatA{from{transform:translate(0,0) scale(1)}to{transform:translate(-20px,25px) scale(1.12)}}
@keyframes zaWowFloatB{from{transform:translate(0,0) scale(1)}to{transform:translate(20px,-16px) scale(1.10)}}
.za-wow-grid{display:grid!important;grid-template-columns:minmax(260px,1fr) minmax(360px,1.55fr)!important;gap:24px!important;align-items:stretch!important}
.za-wow-left{display:flex!important;flex-direction:column!important;justify-content:space-between!important;gap:18px!important}
.za-wow-kicker{font-size:11px!important;font-weight:1000!important;letter-spacing:.16em!important;text-transform:uppercase!important;color:#7dd3fc!important}
.za-wow-title{font-size:36px!important;line-height:1.02!important;margin:8px 0 12px!important;color:#f8fafc!important;font-weight:1000!important;text-shadow:0 0 24px rgba(125,211,252,.12)!important}
.za-wow-score-line{display:flex!important;align-items:center!important;gap:12px!important;flex-wrap:wrap!important}
.za-wow-main-score{display:inline-flex!important;align-items:center!important;font-size:42px!important;font-weight:1000!important;letter-spacing:-.04em!important;line-height:1!important}
.za-wow-main-score.improving{color:#86efac!important;text-shadow:0 0 28px rgba(34,197,94,.26)!important}
.za-wow-main-score.declining{color:#fca5a5!important;text-shadow:0 0 28px rgba(239,68,68,.26)!important}
.za-wow-main-score.stable{color:#bfdbfe!important;text-shadow:0 0 24px rgba(59,130,246,.20)!important}
.za-wow-badge{display:inline-flex!important;align-items:center!important;gap:8px!important;border-radius:999px!important;padding:11px 15px!important;font-size:13px!important;font-weight:1000!important;border:1px solid rgba(148,163,184,.24)!important}
.za-wow-badge.improving{background:rgba(34,197,94,.16)!important;color:#bbf7d0!important;border-color:rgba(34,197,94,.42)!important;box-shadow:0 0 28px rgba(34,197,94,.18)!important}
.za-wow-badge.declining{background:rgba(239,68,68,.17)!important;color:#fecaca!important;border-color:rgba(239,68,68,.42)!important;box-shadow:0 0 28px rgba(239,68,68,.18)!important}
.za-wow-badge.stable{background:rgba(59,130,246,.15)!important;color:#dbeafe!important;border-color:rgba(59,130,246,.34)!important}
.za-wow-insight{margin:0!important;color:#cbd5e1!important;font-size:14px!important;font-weight:800!important;line-height:1.55!important;max-width:560px!important}
.za-wow-meta{display:flex!important;gap:10px!important;flex-wrap:wrap!important}
.za-wow-meta span{border:1px solid rgba(148,163,184,.18)!important;background:rgba(15,23,42,.58)!important;color:#cbd5e1!important;border-radius:999px!important;padding:8px 11px!important;font-size:11px!important;font-weight:900!important}
.za-wow-meta b{color:#f8fafc!important}
.za-wow-chart{border-radius:26px!important;background:rgba(2,6,23,.42)!important;border:1px solid rgba(148,163,184,.17)!important;padding:18px!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.05)!important}
.za-wow-chart-head{display:flex!important;align-items:flex-start!important;justify-content:space-between!important;gap:14px!important;margin-bottom:14px!important}
.za-wow-chart-head strong{display:block!important;color:#f8fafc!important;font-size:15px!important;font-weight:1000!important}
.za-wow-chart-head small,.za-wow-chart-head em{display:block!important;color:#94a3b8!important;font-size:11px!important;font-weight:850!important;font-style:normal!important}
.za-wow-bars{display:flex!important;align-items:flex-end!important;gap:11px!important;height:170px!important;padding:14px!important;border-radius:20px!important;background:linear-gradient(180deg,rgba(15,23,42,.62),rgba(2,6,23,.42))!important;border:1px solid rgba(148,163,184,.12)!important}
.za-wow-bar{flex:1!important;min-width:20px!important;border-radius:999px 999px 9px 9px!important;position:relative!important;height:var(--h)!important;box-shadow:0 0 24px rgba(56,189,248,.16)!important;animation:zaWowBarGrow .95s cubic-bezier(.16,1,.3,1) both!important;transition:transform .28s ease, filter .28s ease!important}
.za-wow-bar.good{background:linear-gradient(180deg,#22c55e,#06b6d4)!important;box-shadow:0 0 26px rgba(34,197,94,.22)!important}
.za-wow-bar.warn{background:linear-gradient(180deg,#f59e0b,#8b5cf6)!important;box-shadow:0 0 26px rgba(245,158,11,.20)!important}
.za-wow-bar.bad{background:linear-gradient(180deg,#ef4444,#7c3aed)!important;box-shadow:0 0 26px rgba(239,68,68,.20)!important}
.za-wow-bar i{position:absolute!important;left:50%!important;bottom:calc(100% + 6px)!important;transform:translateX(-50%)!important;font-style:normal!important;color:#e5e7eb!important;font-size:10px!important;font-weight:1000!important;opacity:.85!important}
@keyframes zaWowBarGrow{from{height:8px;opacity:.22;transform:translateY(14px) scaleX(.82)}78%{transform:translateY(-2px) scaleX(1.03)}to{height:var(--h);opacity:1;transform:translateY(0) scaleX(1)}}
.za-wow-bar:hover{filter:brightness(1.22)!important;transform:translateY(-5px)!important}
.za-wow-bar:hover::after{content:attr(data-tip);position:absolute!important;left:50%!important;bottom:calc(100% + 26px)!important;transform:translateX(-50%)!important;z-index:99999!important;min-width:190px!important;padding:10px 11px!important;border-radius:14px!important;background:rgba(15,23,42,.98)!important;color:#e5e7eb!important;border:1px solid rgba(148,163,184,.24)!important;box-shadow:0 18px 42px rgba(0,0,0,.40)!important;font-size:11px!important;font-weight:850!important;text-align:center!important;line-height:1.35!important}
.za-wow-mini-grid{margin-top:14px!important;display:grid!important;grid-template-columns:repeat(auto-fit,minmax(140px,1fr))!important;gap:11px!important}
.za-wow-mini-grid div{border-radius:18px!important;padding:12px 13px!important;background:rgba(15,23,42,.48)!important;border:1px solid rgba(148,163,184,.14)!important}
.za-wow-mini-grid small{display:block!important;color:#94a3b8!important;font-size:10px!important;font-weight:1000!important;text-transform:uppercase!important;letter-spacing:.08em!important}
...
#zaEliteTrendHero{
  ...
  background:
    ...
    radial-gradient(circle at 84% 8%, rgba(34,197,94,.20), transparent 28%),
    linear-gradient(135deg, rgba(2,6,23,.97), rgba(15,23,42,.94) 46%, rgba(30,41,59,.80))!important;
  box-shadow:0 38px 110px rgba(0,0,0,.56),0 0 78px rgba(56,189,248,.16)!important;
  animation:zaEliteHeroPulse 3.6s ease-in-out infinite alternate;
}
#zaEliteTrendHero.down{
  border-color:rgba(248,113,113,.42)!important;
  box-shadow:0 38px 110px rgba(0,0,0,.56),0 0 78px rgba(239,68,68,.18)!important;
}
#zaEliteTrendHero.stable{border-color:rgba(147,197,253,.34)!important;}
@keyframes zaEliteHeroPulse{
  from{filter:brightness(1);transform:translateY(0);}
  to{filter:brightness(1.07);transform:translateY(-1px);}
}
#zaEliteTrendHero::before{
  content:"";
  position:absolute;
  inset:-2px;
  z-index:-1;
  background:linear-gradient(120deg, transparent 0%, rgba(255,255,255,.13) 42%, transparent 68%);
  transform:translateX(-118%);
  animation:zaEliteShine 5.4s ease-in-out infinite;
}
@keyframes zaEliteShine{0%,58%{transform:translateX(-118%)}100%{transform:translateX(118%)}}
.za-elite-bg-orb{position:absolute;border-radius:999px;filter:blur(22px);opacity:.50;pointer-events:none;z-index:-1}
.za-elite-bg-orb.one{width:160px;height:160px;right:-42px;top:-44px;background:rgba(34,197,94,.34);animation:zaEliteFloatA 7s ease-in-out infinite alternate}
.za-elite-bg-orb.two{width:140px;height:140px;left:-48px;bottom:-48px;background:rgba(99,102,241,.36);animation:zaEliteFloatB 8s ease-in-out infinite alternate}
@keyframes zaEliteFloatA{from{transform:translate(0,0) scale(1)}to{transform:translate(-20px,26px) scale(1.13)}}
@keyframes zaEliteFloatB{from{transform:translate(0,0) scale(1)}to{transform:translate(22px,-18px) scale(1.10)}}
.za-elite-grid{display:grid!important;grid-template-columns:minmax(285px,1fr) minmax(400px,1.62fr)!important;gap:26px!important;align-items:stretch!important}
.za-elite-left{display:flex!important;flex-direction:column!important;justify-content:space-between!important;gap:19px!important}
.za-elite-kicker{font-size:11px!important;font-weight:1000!important;letter-spacing:.16em!important;text-transform:uppercase!important;color:#7dd3fc!important}
.za-elite-left h2{font-size:38px!important;line-height:1.02!important;margin:8px 0 12px!important;color:#f8fafc!important;font-weight:1000!important;text-shadow:0 0 26px rgba(125,211,252,.14)!important}
.za-elite-score-row{display:flex!important;align-items:center!important;gap:13px!important;flex-wrap:wrap!important}
.za-elite-score{display:inline-flex!important;align-items:center!important;font-size:46px!important;font-weight:1000!important;letter-spacing:-.04em!important;line-height:1!important;color:#86efac!important;text-shadow:0 0 34px rgba(34,197,94,.30)!important;animation:zaEliteScorePop .8s cubic-bezier(.16,1,.3,1) both!important}
#zaEliteTrendHero.down .za-elite-score{color:#fca5a5!important;text-shadow:0 0 34px rgba(239,68,68,.30)!important}
#zaEliteTrendHero.stable .za-elite-score{color:#bfdbfe!important;text-shadow:0 0 28px rgba(59,130,246,.22)!important}
@keyframes zaEliteScorePop{from{opacity:.25;transform:scale(.88) translateY(10px)}75%{transform:scale(1.06)}to{opacity:1;transform:scale(1) translateY(0)}}
.za-elite-pill{display:inline-flex!important;align-items:center!important;gap:8px!important;border-radius:999px!important;padding:11px 15px!important;font-size:13px!important;font-weight:1000!important;border:1px solid rgba(34,197,94,.42)!important;background:rgba(34,197,94,.16)!important;color:#bbf7d0!important}
#zaEliteTrendHero.down .za-elite-pill{border-color:rgba(239,68,68,.42)!important;background:rgba(239,68,68,.17)!important;color:#fecaca!important}
#zaEliteTrendHero.stable .za-elite-pill{border-color:rgba(59,130,246,.34)!important;background:rgba(59,130,246,.15)!important;color:#dbeafe!important}
.za-elite-insight{margin:0!important;color:#cbd5e1!important;font-size:14px!important;font-weight:850!important;line-height:1.56!important}
.za-elite-action{border:1px solid rgba(125,211,252,.22)!important;background:rgba(14,165,233,.11)!important;color:#e0f2fe!important;border-radius:18px!important;padding:13px 15px!important;font-weight:950!important;font-size:13px!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.04)!important}
#zaEliteTrendHero.down .za-elite-action{border-color:rgba(248,113,113,.25)!important;background:rgba(239,68,68,.12)!important;color:#fecaca!important}
.za-elite-meta{display:flex!important;gap:10px!important;flex-wrap:wrap!important}
.za-elite-meta span{border:1px solid rgba(148,163,184,.18)!important;background:rgba(15,23,42,.58)!important;color:#cbd5e1!important;border-radius:999px!important;padding:8px 11px!important;font-size:11px!important;font-weight:900!important}
.za-elite-meta b{color:#f8fafc!important}
.za-elite-chart{border-radius:27px!important;background:rgba(2,6,23,.43)!important;border:1px solid rgba(148,163,184,.18)!important;padding:19px!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.05)!important}
.za-elite-chart-head{display:flex!important;align-items:flex-start!important;justify-content:space-between!important;gap:15px!important;margin-bottom:15px!important}
.za-elite-chart-head strong{display:block!important;color:#f8fafc!important;font-size:15px!important;font-weight:1000!important}
.za-elite-chart-head small,.za-elite-chart-head em{display:block!important;color:#94a3b8!important;font-size:11px!important;font-weight:850!important;font-style:normal!important}
.za-elite-bars{display:flex!important;align-items:flex-end!important;gap:12px!important;height:178px!important;padding:15px!important;border-radius:21px!important;background:linear-gradient(180deg,rgba(15,23,42,.64),rgba(2,6,23,.43))!important;border:1px solid rgba(148,163,184,.12)!important}
.za-elite-bar{flex:1!important;min-width:21px!important;border-radius:999px 999px 9px 9px!important;position:relative!important;height:var(--h)!important;animation:zaEliteBarGrow .98s cubic-bezier(.16,1,.3,1) both!important;transition:transform .28s ease, filter .28s ease!important}
.za-elite-bar.good{background:linear-gradient(180deg,#22c55e,#06b6d4)!important;box-shadow:0 0 28px rgba(34,197,94,.23)!important}
.za-elite-bar.warn{background:linear-gradient(180deg,#f59e0b,#8b5cf6)!important;box-shadow:0 0 28px rgba(245,158,11,.21)!important}
.za-elite-bar.bad{background:linear-gradient(180deg,#ef4444,#7c3aed)!important;box-shadow:0 0 28px rgba(239,68,68,.22)!important}
