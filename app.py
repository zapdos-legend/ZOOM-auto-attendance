...
def save_live_data():
    ...
    live_data = {
        "meeting": {
            ...
            "start_time": meeting_info["start_time"].isoformat() if meeting_info.get("start_time") else None,
            "end_time": meeting_info["end_time"].isoformat() if meeting_info.get("end_time") else None,
            "pending_end": meeting_info.get("pending_end", False),
        },
        "participants": {},
    }
    ...
    for name, p in participants.items():
        live_data["participants"][name] = {
            "first_join": p["first_join"].isoformat() if p.get("first_join") else None,
            "last_leave": p["last_leave"].isoformat() if p.get("last_leave") else None,
            "current_join": p["current_join"].isoformat() if p.get("current_join") else None,
...
def finalize_meeting_after_delay(expected_uuid):
    ...
    with state_lock:
        ...
        rows = build_final_rows(total_minutes, PRESENT_PERCENTAGE, member_lookup, HOST_NAME_HINT)
        ...
        meeting_pk = save_meeting_and_attendance(meeting_meta, rows, csv_file, pdf_file)
        ...
        body = (
            ...
            f"Meeting Record ID: {meeting_pk}\n"
        )
        send_email_with_attachment_wrapper(subject, body, [csv_file, pdf_file])

        present_member_names = {
            r["name"].strip().lower()
            for r in rows
            if r["is_member"] == 1 and r["status"] == "PRESENT"
        }

        for member in get_members(active_only=True):
            member_id, name, email, whatsapp, active = member
            if name.strip().lower() not in present_member_names and email:
                ...
                send_email_with_attachment_wrapper(absent_subject, absent_body, [], email)

        top5 = [r for r in rows if r["is_host"] == 0][:5]
        top_text = "\n".join(
            [f"{i+1}) {r['name']} - {r['duration_minutes']} min" for i, r in enumerate(top5)]
        ) or "No attendees"
...
def dashboard():
    ...
    meetings = get_recent_meetings(limit=25)
    analytics = get_dashboard_analytics(HOST_NAME_HINT)

    live_topic = meeting_info.get("topic", "")
    live_meeting_id = meeting_info.get("meeting_id", "")
    present_now = sum(1 for r in live_rows if r["status"] == "LIVE")
    left_count = sum(1 for r in live_rows if r["status"] == "LEFT")

    daily_labels = [row[0] for row in analytics["daily_present_rows"]]
    daily_values = [row[1] for row in analytics["daily_present_rows"]]

    top_labels = [row[0] for row in analytics["top_rows"]]
    top_values = [row[1] for row in analytics["top_rows"]]
    ...
    <body>
        ...
        <p class="small">Permanent link: same Render service. Open anytime using <b>/dashboard</b>.</p>
        ...
        <div class="card">
            <h2>🟢 Live Meeting</h2>
            <div class="grid">
                <div class="metric"><b>Topic</b><br>{{ live_topic or 'No active meeting' }}</div>
                <div class="metric"><b>Meeting ID</b><br>{{ live_meeting_id or '-' }}</div>
                <div class="metric"><b>Present Now</b><br>{{ present_now }}</div>
                <div class="metric"><b>Left</b><br>{{ left_count }}</div>
            </div>

            <h3>Live Participant List</h3>
            <table>
                <tr>
                    <th>Name</th>
                    <th>Status</th>
                ...
                {% endfor %}
                {% if not live_rows %}
                <tr><td colspan="5">No active participants right now.</td></tr>
                {% endif %}
            </table>
        </div>
        ...
        <div class="row">
            ...
            <div class="card">
                ...
                <canvas id="presentChart" height="150"></canvas>
                <br>
